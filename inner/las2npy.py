...
class Point(object):
    ...
    def read_point(self, f, point_data_record_format, num, skip_num_per_read=0):
        """
        读取当前文件中的点数据
        :param skip_num_per_read: 每读一次点云数据跳过的点云数
        :param f:  数据文件
        ...
        while i < num:
            ...
            x = round(x_record * self.x_scale_factor + self.x_offset, 4)
            y = round(y_record * self.y_scale_factor + self.y_offset, 4)
            z = round(z_record * self.z_scale_factor + self.z_offset, 4)
            i += 1
            f.read(offset_bytes+(offset_bytes+12)*skip_num_per_read)
            points.append((x, y, z))

        return points

    def skip_read(self, f, point_data_record_format, num):

        offset_bytes = self.get_offset_bytes(point_data_record_format)
        point_bytes = offset_bytes + 12
        f.read(point_bytes * num)
...
def las_trans(infile, outfile):
    ...
    f.seek(header.offset_to_point_data)
    os.chdir(os.path.dirname(outfile))
    filename = outfile.split('\\')[-1]
    data = points.read_point(f, header.point_data_record_format, num)
    npy = np.array(data)
    np.save(filename, npy)
    # with open(filename.split('.')[0]+'.txt', 'a+') as f_out:
    #     for item in data:
...
if __name__ == '__main__':
    sor = r'H:\data\sparsed_rotated_las'
    des = r'H:\data\output\npy'
    for file in os.listdir(sor):
        inpath = os.path.join(sor, file)
        outpath = os.path.join(des, file.split('.')[0]+'.npy')
        las_trans(inpath, outpath)
        print(file)
